...
class Field {
  ...
  public float sroot(float x) {
    return x<0?-50*sqrt(abs(x)):50*sqrt(x);
  }
  PVector delta;
  PVector Pnode = new PVector(0, 0);
  public void stim(PVector p, float mag) {
    for (int i =-1; i++ <Ematrix.length-1; ) {
      for (int  j=-1; j++ <Ematrix[0].length-1; ) {
        for (int n =0; n<1; n++) {
          for (int m=0; m<1; m++) {
            delta = PVector.sub(Pnode.set((i+n)*R, (j+m)*R), p);
            Ematrix[i][j].add(delta.mag()>1.41421f*R?delta.setMag(mag/delta.magSq()):delta.setMag(0)); //don't ask
          }
        }
      }
    }
  }
  public PVector [][] set(PVector [][] m) {
...
class Particle {
    ...
    if (accessX<F.Ematrix.length-1&&accessY<F.Ematrix[0].length-1) {
      if (accessX>=0&&accessY>=0) {
        ...
        if (particles) {
          strokeWeight(1);
          if (charge<0) {
            stroke(0.5f, 1, 1, 0.9f);
          } else {
            stroke(0.03f, 1, 1, 0.9f);
          }
          point(pos.x, pos.y);
          strokeWeight(1);
        }
  ...
  public PVector del(PVector P, float q) {
    float xdim1, xdim2;
    float ydim1, ydim2;
    PVector result;
    int i = accessX;
